...
extern uint32_t _eotext;
extern uint32_t _sodata;
extern uint32_t _eodata;
extern uint32_t _sobss;
extern uint32_t _eobss;

//prototype of main
int main(void);

// weak function definition and alias
...
// default handler implementation of interrupts
// will be aliased as weak functions and overriden by program code
void Default_Handler(void)
{
	while(1);
}

// the reset handler code gets executed initially
void Reset_Handler(void)
{
	uint32_t size = (uint32_t)&_eodata - (uint32_t)&_sodata;

	uint8_t *pDst = (uint8_t*)&_sodata;
	uint8_t *pSrc = (uint8_t*)&_eotext;
	...
	size = (uint32_t)&_eobss - (uint32_t)&_sobss;
	pDst = (uint8_t*)&_sobss;
